...
def main() -> None:
    ...
    print(f"4b. Live Polymarket collection: {markets_result['polymarket_collect_status']} "
    ...
    team_names = {g["event_id"]: (g["home_team"], g["away_team"]) for g in state.tonight.iter_rows(named=True)}
    report = []
    for g in state.tonight.iter_rows(named=True):
        ...
        if event_id in games_by_event:
            ...
            report.append({
                ...
                "status": state.skipped[event_id], "decision": None,
            })
    ...
    out_path.write_text(json.dumps({
        "date": target_date, "model_train_games": predict_result["train_games"],
