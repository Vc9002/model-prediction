...
from model_prediction.learned_forward import _FEATURE_PROVIDERS  # noqa: E402
from model_prediction.production_canary import load_production_config  # noqa: E402
from model_prediction.production_registry import compute_artifact_hash  # noqa: E402
from model_prediction.runtime_paths import RuntimePaths  # noqa: E402
...
def _ledger_stats(conn: sqlite3.Connection, model_id: str) -> dict:
    rows = conn.execute(
        "SELECT status, count(*), max(settled_at_utc) FROM ledger_records "
        "WHERE model_id = ? GROUP BY status",
        (model_id,),
    ).fetchall()
    stats = {status: {"n": n, "last_settled": last} for status, n, last in rows}
    total = conn.execute(
        "SELECT count(*) FROM ledger_records WHERE model_id = ?", (model_id,)
    ).fetchone()[0]
...
def _sport_freshness(sport: str) -> dict:
    mapping = {
        "mlb": "mlb_games_all.jsonl", "wnba": "wnba_games_all.jsonl",
        "nba": "nba_games_all.jsonl", "nfl": "nfl_games_all.jsonl",
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            continue
    days = (datetime.now(UTC).date() - datetime.strptime(latest, "%Y-%m-%d").date()).days if latest else None
...
def main() -> int:
    ...
    for entry in sorted(models, key=lambda e: str(e.get("sport"))):
        ...
        if artifact_rel:
            ...
            if not artifact_path.is_file():
                ...
                if artifact:
                    ...
                    # serving coverage
                    feature_names = (
                        (artifact.get("market_models") or {}).get("moneyline") or {}
                    ...
                    _lf._init_providers()
                    inline = {
                        "elo_probability", "trend_gap", "defensive_trend_gap",
                        "consistency_gap", "hot_cold_gap", "starter_era_gap",
                        "starter_fip_gap", "starter_kbb_gap",
                        "bullpen_weakness_gap", "bullpen_fatigue_gap",
                        "probable_starter_era_gap", "rest_disparity",
                        "back_to_back_gap", "games_last_7_gap", "schedule_available",
                    }
                    missing_providers = [
                        f for f in feature_names
                        if f not in _FEATURE_PROVIDERS and f not in inline
                    ]
        ...
        print(f"[{marker}] {sport_l:10s} {detail.get('model_version')} "
              f"| ledger rows: {detail['ledger']['total']} | "
