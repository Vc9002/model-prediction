...
def _run_settlement(ledger: ShadowLedger, run_id: str) -> dict[str, Any]:
    sb_path = Path(DATA_ROOT) / "normalized/mlb/scoreboard.parquet"
    if not sb_path.exists():
        raise FileNotFoundError(f"{sb_path} not found; run the MLB scoreboard collector first")
    ...
    return {
        "settlements_recorded": n_settled,
        "skipped_not_final": n_skipped_not_final,
        "closing_dates_attempted": closing_attempted,
        "closing_prices_recorded": closing_found,
...
def main() -> None:
    ...
    run_id = ledger.record_run(SPORT, run_type="settlement", mode="fresh")
    try:
        result = _run_settlement(ledger, run_id)
