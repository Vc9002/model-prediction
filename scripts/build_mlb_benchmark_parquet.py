...
def main() -> None:
    ...
    for method, m in dist["oof_summary"].items():
        rows.append({
            "sport": "mlb", "model": method, "market_type": "moneyline", "selection": None,
            "line": None, "calibrated": False, "calibration_method": None,
            "n": dist["n_oof"], "log_loss": m["log_loss"], "brier": m["brier"], "ece": m["ece"],
            "status": STATUS, "dataset_hash": dataset_hash, "source_file": "mlb_distribution_comparison.json",
        })

    # Raw (uncalibrated) moneyline OOF comparison across model families (Task 13).
    for model, m in score["moneyline_oof_summary"].items():
        rows.append({
            "sport": "mlb", "model": model, "market_type": "moneyline", "selection": None,
            "line": None, "calibrated": False, "calibration_method": None,
            "n": score["n_moneyline_oof"], "log_loss": m["log_loss"], "brier": m["brier"], "ece": None,
            "status": STATUS, "dataset_hash": dataset_hash, "source_file": "mlb_score_model_comparison.json",
        })
    ...
    for model, by_line in score["totals_oof_summary"].items():
        for line_str, entry in by_line.items():
            rows.append({
                "sport": "mlb", "model": model, "market_type": "total", "selection": "over",
                "line": float(line_str), "calibrated": False, "calibration_method": None,
                "n": entry["n"], "log_loss": entry["log_loss"], "brier": entry["brier"], "ece": None,
                "status": STATUS, "dataset_hash": dataset_hash, "source_file": "mlb_score_model_comparison.json",
            })
    ...
    for model, by_line in score.get("spread_oof_summary", {}).items():
        for line_str, entry in by_line.items():
            rows.append({
                "sport": "mlb", "model": model, "market_type": "spread", "selection": "home",
                "line": float(line_str), "calibrated": False, "calibration_method": None,
                "n": entry["n"], "log_loss": entry["log_loss"], "brier": entry["brier"], "ece": None,
                "status": STATUS, "dataset_hash": dataset_hash, "source_file": "mlb_score_model_comparison.json",
            })
    ...
    for model, model_result in calib["models"].items():
        for method, m in model_result["cross_fit_results"].items():
            rows.append({
                "sport": "mlb", "model": model, "market_type": "moneyline", "selection": None,
                "line": None, "calibrated": True, "calibration_method": method,
                "n": m["n_eval_total"], "log_loss": m["log_loss"], "brier": m["brier"], "ece": m["ece"],
                "status": STATUS, "dataset_hash": dataset_hash, "source_file": "mlb_calibration_comparison.json",
            })
    ...
    for method, m in ens["meta_cross_fit_results"].items():
        rows.append({
            "sport": "mlb", "model": method, "market_type": "moneyline_ensemble", "selection": None,
            "line": None, "calibrated": True, "calibration_method": "meta_cross_fit",
            "n": m["n_eval_total"], "log_loss": m["log_loss"], "brier": m["brier"], "ece": None,
            "status": STATUS, "dataset_hash": dataset_hash, "source_file": "mlb_calibrated_ensemble_comparison.json",
        })
    ...
    print(df.select("model", "market_type", "line", "calibrated", "n", "log_loss").sort("market_type", "model"))
