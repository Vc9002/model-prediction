...
| Soccer | `soccer-poisson-dc-v1` — shadow_qualified (operator override), 62.5% locked-holdout; no persisted qualification artifact (`config/model.yaml` comment only) | — | `_BasicEloAdapter` (binary, semantically inadequate for a 3-way sport — deliberate foundation stage, not a bug, per `ARCHITECTURE_CORRECTION.md`) | **FITTED** — `soccer-poisson-dc-rebuild-v1` (Dixon-Coles Poisson, MLE). Data foundation expanded to 19 leagues (from 6). Independent MLE fit from ESPN site v2 data; no incumbent constants. `production_allowed=false` (capture-time-only provenance). See `docs/model_audit/models/SOCCER_POISSON_DC_REBUILD_V1.md`. |
