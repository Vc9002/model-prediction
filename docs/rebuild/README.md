...
`rebuild-data --sport soccer`, and `rebuild-data --sport tennis` are also
real (data ingestion only -- feature-engineering/model-baseline work is a
separate, not-yet-made decision for all four). Every other sport on
`rebuild-data`, and every sport on `rebuild-model`, currently reports
`NOT_IMPLEMENTED` -- see `OPERATIONS.md`.
