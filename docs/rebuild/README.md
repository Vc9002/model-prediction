...
`MLB_V3_DATA.md`); `rebuild-data --sport wnba` is also real (data
ingestion only -- feature-engineering/model-baseline work is a separate,
not-yet-made decision). Every other sport on `rebuild-data`, and every
