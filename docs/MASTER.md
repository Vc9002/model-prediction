...
- **Comprehensive Defect Audit & Gaps Catalog** ([`docs/SYSTEM_DEFECTS_AND_GAPS_AUDIT.md`](SYSTEM_DEFECTS_AND_GAPS_AUDIT.md)):
  ...
  - **Upstream Feed Outages (updated 2026-09-02)**: The dead Odds API dependency was removed. `api_football.py` remains unprovisioned, so soccer historical capture is stale.
  - **Polymarket Market Coverage (updated 2026-09-01)**: NRFI/YRFI quote ingestion is wired through game-detail hydration; F5 result-target coverage remains missing.
...
- **GOVERNING INVARIANT (HARD-CODED & PINNED BY TEST `tests/test_ledger_invariants.py`)**:
  ...
  - **Gated Ledgers** (`Production Ledger`, `Gated Research`, and `Polymarket Edge Ledger`): Exclusively enforce quality gates, spread price caps, and Kelly risk controls.
- **Gated Fixes & Risk Controls**:
  1. **Spread Gating & Price Caps** (`rebuild/decision.py`, `polymarket_kelly.py`): Enforced a $+3.5\text{pp}$ minimum edge hurdle and a $60.0¢$ price ceiling on spread/runline contracts to eliminate the $-11.3\%$ ROI spread drag.
  2. **Real-Edge Quarter-Kelly Sizing** (`units.py`): Replaced legacy distance-from-50% scaling (`abs(p - 0.5)`) with true market-relative edge ($\text{adjusted\_prob} - \text{market\_prob}$), stopping $65\%$ favorites with thin $+1\%$ edges from getting over-staked at $2.0\text{U}$.
  3. **Correlation-Aware Exposure Caps** (`portfolio/polymarket_kelly.py`, `portfolio/polymarket_scanner.py`): Capped same-game derivatives (moneyline + spread + total) to one aggregate exposure bucket.
...
- **Distribution & Feature Enhancements**:
  - Implemented `total_over_under_probability()` in `total_score.py` for Gamma-Poisson / Negative-Binomial distribution over/under probabilities.
  - Implemented disk-backed binary caching with mtime validation in `features/starter_history.py` (225x faster cold start).
  - Implemented opponent-quality (SOS) adjusted rolling pitcher ERA (`starter_sos_adjusted_era` / `starter_sos_era_gap_live`).
  - Implemented shared cross-sport travel timezone displacement fatigue in `features/schedule_load.py`.
  - Implemented 3-way KBO/NPB multinomial probability model (`kbo_npb_multinomial_probabilities` in `international_baseball.py`).
- **Portfolio & Observability**:
  - Implemented `SharpLeadLagAnalyzer` in `portfolio/lead_lag.py` detecting bookmaker latency.
  - Implemented `ExecutionRehearsalRunner` in `portfolio/execution_rehearsal.py` with `/api/polymarket/rehearsal`.
  - Implemented stateful Hypothesis testing of ledger APIs in `tests/test_ledger_stateful.py`.
  - Added `/api/drawdown` (realized P&L / HWM curve), `/api/export/picks` (streamable CSV export), and `/api/picks/explanation` (per-feature contribution breakdown).
  - Deployed Platt-scaled temperature calibration ($T=1.18$) for Esports.
- **Dashboard & UI**:
  - Fixed tab switching and click event binding for Flat, Research, and Gated Research ledgers.
  - Excluded archived pre-change records across badge counts and table renders.
**Session 2026-08-20 — MLB YRFI/NRFI model & research holdout pass; WNBA Four Factors; cross-market consistency; shared meta-calibrator; lineup wake daemon; worktree purge**: Operator directed infrastructure & project work, MLB YRFI/NRFI research/model creation, and literature analysis. **MLB YRFI/NRFI**: Created PIT feature pipeline (`features/yrfi_nrfi.py`: starting pitcher 1st-inning ERA/FIP/WHIP with empirical Bayes shrinkage, top 3 lineup offensive priors, park factor, and independent half-inning Poisson $\lambda_{\text{top}}, \lambda_{\text{bot}}$ calculations), hybrid predictive engine (`models/mlb_nrfi.py`: decomposed Poisson + logistic model with fair odds and EV edge), and unit tests (`tests/test_mlb_nrfi.py`). Walk-forward holdout research backtest across 6,610 games (`scripts/mlb_nrfi_research.py`): **Holdout Brier improved from 0.25113 to 0.24844 ($\Delta\text{Brier} = -0.00268$)** and **Log Loss from 0.69541 to 0.69007 ($\Delta\text{LL} = -0.00534$)**. **WNBA Pace & Four Factors**: Created `features/wnba_pace_four_factors.py` implementing Dean Oliver's Four Factors for WNBA 40m regulation with Empirical Bayes shrinkage ($n=8$), and projected game totals. **Cross-Market Consistency**: Created `cross_market_consistency.py` validating monotonicity ($P(\text{Cover } -1.5) \le P(\text{Moneyline Win})$) and complementarity ($P(\text{Over}) + P(\text{Under}) = 1.0$). **Shared Meta-Calibrator**: Created `meta_calibrator.py` with multi-sport Platt scaling and Isotonic Regression. **Dashboard Observability**: Added `/api/clv` (rolling 30-day CLV and closing beat rate) and `/api/capture_health` (7-day BBO snapshot freshness) in `dashboard/status.py` and `routes.py`. **Wake Planner Root Daemon**: Created `ops/launchd/com.vc.mlb-lineup-wake-planner.plist` for root installation to automate slate-following `pmset` wakes ~35m pre-pitch. **Push Notifications**: Added `notify_operator()` in `run_supervisor.py` (macOS `osascript` + Slack webhook). **Hygiene & Type Safety**: Purged 11 orphaned/broken worktrees from `$HOME`; created `src/model_prediction/py.typed` and configured mypy overrides in `pyproject.toml`. Full suite verified live: **1,938 tests pass / 3 skipped / 0 failed; ruff clean (0 findings).**
...
### 🟠 Important — DD-3 through DD-7 fixed
...
| DD-5 | **`dashboard_server.py` at 5,358 lines** | `dashboard_server.py` | Single monolithic file with ~15 POST routes + ~20 GET routes dispatched via manual `if/elif parsed.path` chain. Every new feature lands here | **Fixed 2026-08-20.** `dashboard_server.py` split into a `src/model_prediction/dashboard/` package (`common.py`, `picks.py`, `evidence.py`, `status.py`, `matrix.py`, `backtests.py`, `orders.py`, `jobs.py`, `routes.py`, plus the pre-existing `data_service.py`) by domain responsibility, mirroring DD-6's `cli/` pattern. `dashboard_server.py` is now a ~250-line thin entrypoint (`main()` + a full re-export shim) so every old `dashboard_server._X` import/monkeypatch keeps resolving. Fixed two real bugs surfaced by the split: `ROOT` was computed relative to the new module's own directory instead of the repo root (broke every config/data path), and a `global _DASHBOARD_CACHE` reassignment in `picks.py` referenced a name owned by `common.py` (NameError on `/api/picks` — reassigning a module-level name across modules doesn't share state the way mutating a dict/list in place does; fixed via a `_get_dashboard_cache()` accessor common.py owns). `tests/conftest.py`'s new `patch_dash` fixture patches a name across every bound module (submodules each import their own copy of shared names) instead of only `dashboard_server`; also works around `status`/`matrix`/`backtests` submodules being shadowed by same-named functions the package `__init__` re-exports. Also cleared the ~34 pre-existing ruff findings DD-10 had deliberately deferred into this file (unnecessary `datetime` `.replace("Z", "+00:00")` calls now that `fromisoformat` handles `Z` natively, explicit `subprocess.run(check=False)` to match existing manual returncode checks, an executable-bit/shebang mismatch, and noqa-annotated blind excepts at API/route/cache boundaries, matching the style already used elsewhere in the file) — no behavior changes. Full suite green (1917 passed, 3 skipped), ruff clean on every touched file. Manually verified the live server: started it and confirmed `/api/status`, `/api/picks`, `/api/matrix` all return 200 with real data. |
...
| DD-7 | **Zero TODO/FIXME/HACK markers in entire codebase** | All source files | No visible technical debt tracking — problems found in code reviews or audits are only tracked externally (in MASTER.md, DEBUG.md, CHECKLIST.md), not in the code where they'd be visible to anyone reading it | **Fixed 2026-08-16** (commit `df05fb3`) — in-code debt markers added; this table's header hadn't been updated to reflect it until 2026-08-19. |
...
| DD-9 | **No pre-commit ruff hook** | Project config | Lint violations accumulate between CI runs. 126 ruff findings exist (mostly EXE002 shebangs — cosmetic) | **Fixed 2026-08-16** (commit `df05fb3`) — `.pre-commit-config.yaml` (ruff + ruff-format) installed and live in `.git/hooks/pre-commit`. |
| DD-10 | **126 ruff findings (baseline) — 79 are EXE002 shebang** | `tests/` directory | 79 test files have executable bits set without shebangs — harmless but noisy. Remaining 47 are style/formatting | **Re-baselined and cleared 2026-08-19** (commit `c1d2bac`) — 104 findings had reaccumulated since the 08-16 sweep (new code landing between commits, hook only guards new commits going forward). All cleared outside `dashboard_server.py`, whose findings were deferred to DD-5's split to avoid touching the same file mid-refactor — DD-5 (2026-08-20) cleared those too. Full suite still 1910 passed, 3 skipped. |
