...
- [x] **Per-League Soccer Dixon-Coles Registry (`soccer_leagues.py`)**: Split monolithic global soccer model into independent per-league Dixon-Coles parameter sets (EPL, La Liga, Bundesliga, Serie A, MLS, UCL) with league-specific home advantage, correlation, and optimal decay search $\xi^*$. → DONE: `LeagueDixonColesRegistry` & `test_soccer_leagues.py`.
- [x] **WNBA v5 Possession $\times$ Points-per-Possession (PPP) Model (`wnba_possession.py`)**: Model fundamental pace (possessions per 40 min) and four-factors offensive/defensive efficiency ratings with Empirical Bayes shrinkage toward WNBA league baseline; derive joint distributions for ML, spread margin, and totals. → DONE: `WNBAPossessionEngine` & `test_wnba_possession.py`.
- [x] **NFL Pre-Season & In-Season Probability Calibrator (`nfl_calibration.py`)**: Multi-method calibration (Temperature, Platt, Isotonic) with week-specific shrinkage damping for early-season high-variance weeks (Weeks 1-3) and full Brier/LogLoss/ECE metrics. → DONE: `NFLCalibrator` & `test_nfl_calibration.py`.
- [x] **Tennis Barnett & Clarke Opponent-Adjusted Point-Level Markov Engine (`tennis_markov.py`)**: Solves exact point -> game -> tiebreak -> set -> match absorbing Markov chains with surface adjustments, Set 1 retirement rule outputs, and Bo3/Bo5 format scaling. → DONE: `TennisMarkovEngine` & `test_tennis_markov.py`.
- [x] **Tennis v2 Dynamic Surface & Inactivity Shrinkage Model (`tennis_v2.py`)**: Replaces static 60/40 surface weighting with Empirical Bayes surface shrinkage ($w_{\text{surface}} = 0.75 \times \frac{N}{N+15}$) and 45-day inactivity rust decay. → DONE: `TennisV2Model` & `test_tennis_v2.py`.
- [x] **MLB Point-in-Time Umpire Strike Zone Feature Engine (`umpire_state.py`)**: Ingests umpire-specific Called Strike Rate Above Expected (CSAE) and run-environment multipliers with Empirical Bayes shrinkage over 30 games. → DONE: `PointInTimeUmpireEngine` & `test_umpire_state.py`.
- [x] **Polymarket US Binary Edge Gate & Quarter-Kelly Sizer (`polymarket_kelly.py`)**: Closed-form binary Quarter-Kelly ($0.25 \times \frac{P - A}{1 - A}$), hard $+2.5\%$ minimum edge gate, inside-spread maker limit orders, and 0.50 tie settlement pricing. → DONE: `PolymarketKellyEngine` & `test_polymarket_kelly.py`.
- [x] **Title-Specific Esports Engines (`esports_cs2.py`, `esports_lol.py`, `esports_valorant.py`, `esports_dota2.py`, `esports_r6.py`)**: CS2 map veto and pistol round momentum (+2.15 rounds); LoL Blue side first-pick bias (+3.5%) and GD@15 objective control; Valorant map pools and agent economy; Dota 2 Radiant side bias (+2.5%), NW@15, and Bo2 0.50 tie pricing; R6 9-map tactical defense engine. → DONE: All 5 title engines & tests.
- [x] **International Baseball (KBO & NPB) Starter & 0.50 Tie Settlement Engine (`intl_baseball.py`)**: High-scoring KBO vs pitcher-friendly NPB baselines with starter FIP/K-BB% Empirical Bayes shrinkage and 12-inning tie pricing ($E[\text{Payout}] = P(W) + 0.5 \times P(\text{Tie})$). → DONE: `IntlBaseballEngine` & `test_intl_baseball.py`.
- [x] **Polymarket US Unified Multi-Sport Dispatcher & Scanner (`polymarket_dispatcher.py`, `polymarket_scanner.py`, `scripts/scan_polymarket_slate.py`)**: Central execution dispatcher and CLI scanner parsing prospective Polymarket CLOB snapshot streams, routing across 11 leagues into domain models, enforcing the $+2.5\%$ minimum edge filter and creating binary Quarter-Kelly order tickets. → DONE: `PolymarketDispatcher`, `PolymarketSlateScanner`, `scan_polymarket_slate.py` & tests.
- [x] **Operations Dashboard Polymarket UI Wiring (`dashboard.html`, `routes.py`, `test_dashboard_polymarket.py`)**: Added dedicated "Polymarket US" Edge Scanner tab to the local operations dashboard with live sport filtering, bankroll/min-edge controls, maker limit toggles, KPI overview, and interactive Quarter-Kelly order ticket tables. → DONE: `/api/polymarket/scan` endpoint & full UI suite.
