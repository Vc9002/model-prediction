...
class TestMarketStateFailsClosed:
    ...
    def test_post_start_quote_is_no_bet(self):
        candidate = _market(ask=0.50).__class__(
            **{
                **_market(ask=0.50).__dict__,
                "observed_at_utc": "2026-08-06T22:10:00+00:00",
                "event_start_utc": "2026-08-06T22:10:00+00:00",
            }
        )
        decision = decide_team_market(_forecast(), candidate)
        assert decision.action == "NO_BET"
        assert decision.reason_code == POST_START_QUOTE

    def test_stale_quote_is_no_bet(self):
        decision = decide_team_market(_forecast(), _market(ask=0.50, quote_age=301.0))
        assert decision.action == "NO_BET"
        assert decision.reason_code == STALE_QUOTE
        assert decision.units == 0

    def test_ambiguous_event_is_no_bet(self):
        candidate = _market(ask=0.50).__class__(
            **{**_market(ask=0.50).__dict__, "event_match_ambiguous": True}
        )
        decision = decide_team_market(_forecast(), candidate)
        assert decision.action == "NO_BET"
        assert decision.reason_code == AMBIGUOUS_EVENT
