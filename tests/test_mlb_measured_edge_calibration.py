...
def _write_margin_artifact(path, scale: float, offset: float):
    artifact = {
        "model_version": "measured-edge-margin-v1",
        "base_score_model_version": "mlb-analyst-poisson-trend-v0.2",
        ...
        "calibration_version": "measured-edge-margin-v1",
    }
...
@pytest.fixture
def spec():
    return load_formula_spec(PROJECT_ROOT / "config/models/mlb-analyst-poisson-trend-v0.2.yaml")
...
def test_heavy_shrinkage_calibration_is_allowed_when_correctly_centered(tmp_path, spec):
    ...
    path = tmp_path / "measured-edge-margin-v1.json"
...
def test_production_margin_and_totals_artifacts_load_cleanly(spec):
    margin = MeasuredEdgeMarginModel(
        PROJECT_ROOT / "config/models/measured-edge-margin-v1.json", spec
    )
    totals = MeasuredEdgeTotalsModel(
        PROJECT_ROOT / "config/models/measured-edge-totals-v1.json", spec
    )
