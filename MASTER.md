...
**Generated**: 2026-08-02 | **Last verified against live code**: 2026-08-04 |
...
| Metric | Value |
|---|---|
| Tests | **699 pass, 0 fail** (re-verified 2026-08-04, final — after F-63; was 694 after F-62, 692 after F-58 through F-61, 686 after F-53 through F-57, 675 after the deep-debug session, 673 after P1 cleanup, 670 after F-47/F-48, 654 before that) |
| Ruff | **126 findings in `src/ tests/`** (current baseline as of 2026-08-04, final; confirmed unchanged by every fix this session via direct before/after comparison, not `git stash` guessing — see each F-5x/F-6x entry; F-58/F-59/F-60/F-61's own new tests are clean, F-58/F-59's touched source files introduced and then fixed one new transient `I001` import-sort finding each, not part of the baseline). |
...
| Git | Single `main` branch. **HEAD is `31d3b7c` as of 2026-08-04, later (pre-final-audit) — pushed to `origin/main`.** F-58 through F-61 are committed separately, see the next commit after `31d3b7c`. Both this session's commits (`face73f`, `31d3b7c`) and the concurrent P1 session's fixes landed together (shared working tree, could not be cleanly split — see the commit messages for the full breakdown). Older rows below claiming HEAD `01634e2`/dirty-working-tree are stale. |
| Last pushed commit | **`31d3b7c` as of the last push; F-58 through F-61 committed after this and pushed in the same session.** The `52da4a5` figure below is stale. |
...
- **Ledger/audit atomicity**: the real code appends the audit event *before* the ledger write, the opposite of what was originally claimed. True cross-file atomicity across separate ledger/audit files still doesn't exist (a lower-severity, still-real architectural gap — no failure-injection tests either), but the specific "silent mutation with no audit record" failure mode never happened.
...
| P1-17 | MLB totals over-picks "over" (27/38, 71%) | Root-caused: elasticities near-zero (λ barely moves from league average), side selection before calibration. Fix needs re-fitting elasticities against totals outcomes + moving selection after calibration — not a quick patch |
...
|---|---|---|---|
| DD-1 | **`suppress(Exception)` swallows everything** | `international_baseball.py:533` | Silently discards ALL exceptions including `KeyboardInterrupt` and `SystemExit` — if the data source fails, the daily pipeline reports success with zero games |
| DD-2 | **14 `suppress(DuplicatePickError)` silently drop real duplicates** | `cli.py` (14 occurrences across `_forecast_mlb`, `_forecast_mlb_totals_flat`, `_forecast_learned_sport`, `_forecast_soccer_sport`, `_forecast_tennis_sport`) | A genuine duplicate pick (same event, market, line, model_version — different selection or same-day re-forecast with different probability) is silently discarded with no audit trail. Operator cannot distinguish "model chose not to pick" from "model picked but ledger already had this market" |
...
| DD-4 | **8 orphaned feature modules (469 lines of dead code)** | `features/confidence_gate.py` (111 lines), `guaranteed_signal.py` (63), `head_to_head.py` (61), `lineup_strength.py` (40), `market_signals.py` (64), `starting_pitcher.py` (58), `tennis_surface.py` (72) — never imported anywhere | Dead code creates false signal about what's active, wastes CI time, and misleads new contributors |
...
| DD-6 | **`cli.py` at 4,264 lines (+321 since last audit), 48 subcommands** | `cli.py` | Largest module in the codebase, near-zero test coverage, all commands in one file |
...
| # | Issue | Location | Impact |
|---|---|---|---|
| DD-8 | **Multiple hardcoded thresholds scattered across code** | `units.py` (min_edge=0.02, unit_increment=0.25), `confidence_gate.py` (target_hit_rate=0.60), `guaranteed_signal.py` (edge≥0.08, spread≤0.05), `tennis.py` (uncertainty=0.05), `esports.py` (min_observations=50, min_accuracy=0.60), `roadmap_challenger.py` (p≤0.05) | Thresholds duplicated across modules with no single source of truth — changing a policy requires finding every copy |
