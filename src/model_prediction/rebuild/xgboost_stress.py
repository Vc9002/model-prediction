...
def nested_xgboost_fold(
    ...
    *,
    ...
    return NestedFoldResult(
        ...
        inner_train_n=len(X_inner_train),
        inner_val_n=len(X_inner_val),
        outer_train_n=n,
        outer_val_n=len(X_outer_val),
    )
...
def run_stress_tests(
    ...
    one_tick = sum(
        max(-1.0, t.get("pnl", 0) * 0.90)
        for t in trades  # 10% worse entry
    )
    results.append(
        StressTestResult(
            "one_tick_worse",
    ...
    # 2. Two ticks worse
    two_ticks = sum(max(-1.0, t.get("pnl", 0) * 0.80) for t in trades)
    results.append(
        StressTestResult(
            "two_ticks_worse",
            base_pnl,
    ...
    shrunk_pnl = sum(t.get("pnl", 0) * (1.0 - abs(t.get("edge", 0)) * 0.5) for t in trades)
    results.append(
        StressTestResult(
            "probability_shrinkage",
            base_pnl,
