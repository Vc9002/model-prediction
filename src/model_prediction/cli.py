...
from .runtime_paths import RuntimePaths, rolling_models_root
...
def _forecast_mlb_totals_flat(
    ...
    *,
    ...
    if preflight_only:
        result["_prepared_rows"] = planned_rows
        result["_observed_at"] = observed_at
    return result


def _commit_preflighted_mlb_totals(
    preflight: dict[str, Any], flat_ledger, main_ledger=None
) -> dict[str, Any]:
    """Persist a fully validated plan without rebuilding external inputs."""
    planned_rows = preflight["_prepared_rows"]
    observed_at = preflight["_observed_at"]
    logged = []
    duplicates = []
    main_duplicates = []
    for request, eligibility in planned_rows:
        try:
            logged.append(flat_ledger.append_evaluated(request, eligibility, now=observed_at))
            if main_ledger is not None and eligibility.decision == "CALL":
                duplicate = _append_secondary_ledger(
                    main_ledger,
                    request,
                    eligibility,
                    observed_at,
                    "mlb_totals:main_ledger",
                )
                if duplicate is not None:
                    main_duplicates.append(duplicate)
        except DuplicatePickError as error:
            duplicates.append(error.pick_id)
    result = {key: value for key, value in preflight.items() if not key.startswith("_")}
    result.update(
        logged=len(logged),
        logged_pick_ids=[row["pick_id"] for row in logged],
        duplicate_pick_ids=duplicates,
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        if args.command == "init-ledger":
            ...
            blend_artifact_path = getattr(args, "market_blend_policy_artifact", None)
            blend_report_path = getattr(args, "market_blend_policy_report", None)
            if (blend_artifact_path is None) != (blend_report_path is None):
                raise MarketBlendBlockedError(
                    "blend activation requires both policy artifact and gate report"
                )
            if blend_artifact_path is not None and (
                not is_flat or getattr(args, "all", False) or sports != ["mlb"]
            ):
                raise MarketBlendBlockedError(
                    "Stage 1 blend flags are supported only by flat-forecast --sport mlb"
                )
            mlb_totals_preflight = None
            flat_rows_before_mlb_replace = None
            if is_flat and sports == ["mlb"]:
                flat_rows_before_mlb_replace = flat_ledger.rows()
                mlb_totals_preflight = _forecast_mlb_totals_flat(
                    args.date,
                    True,
                    config,
                    registry,
                    ...
                    flat_ledger,
                    audit,
                    blend_policy_artifact_path=blend_artifact_path,
                    blend_policy_report_path=blend_report_path,
                    preflight_only=True,
                )
            if is_flat:
            ...
            for sport in sports:
                ...
                if selected_model == "legacy-measured-edge":
                    ...
                    if is_flat and sport == "mlb":
                        try:
                            results["mlb_totals"] = _commit_preflighted_mlb_totals(
                                mlb_totals_preflight, flat_ledger
                            )
                        except Exception:
                            if flat_rows_before_mlb_replace is not None:
                                flat_ledger.restore_rows_for_rollback(
                                    flat_rows_before_mlb_replace,
                                    "MLB flat replacement commit failed",
                                )
                            raise
                else:
            ...
            def _mlb_totals_task() -> None:
                ...
                except MarketBlendBlockedError:
                    raise
                except Exception:
