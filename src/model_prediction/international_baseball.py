...
def kbo_npb_multinomial_probabilities(
    home_rating: float,
    away_rating: float,
    home_advantage: float = 24.0,
    starter_gap_rating: float = 0.0,
    park_factor: float = 1.0,
    baseline_tie_rate: float = 0.045,
) -> dict[str, float]:
    """Calculate explicit 3-way P(home)/P(tie)/P(away) multinomial distribution.

    Accounts for 12-inning regular-season tie rules in KBO/NPB, starter adjustments,
    and park factors, then calculates fair Polymarket binary settlement values.

    Returns
    -------
    dict with 'p_home_win', 'p_tie', 'p_away_win', 'p_polymarket_home', 'p_polymarket_away'.
    """
    effective_home_diff = (home_rating + home_advantage + starter_gap_rating) - away_rating
    p_decisive_home = 1.0 / (1.0 + 10.0 ** (-effective_home_diff / 400.0))

    # Tie probability varies with expected scoring / park factor (lower scoring -> higher tie rate)
    adjusted_tie_rate = max(0.01, min(0.12, baseline_tie_rate / max(0.5, park_factor)))

    p_home_win = p_decisive_home * (1.0 - adjusted_tie_rate)
    p_away_win = (1.0 - p_decisive_home) * (1.0 - adjusted_tie_rate)
    p_tie = adjusted_tie_rate

    # Polymarket US settlement rule: Tie settles at 0.50
    p_polymarket_home = p_home_win + 0.5 * p_tie
    p_polymarket_away = p_away_win + 0.5 * p_tie

    return {
        "p_home_win": round(p_home_win, 6),
        "p_tie": round(p_tie, 6),
        "p_away_win": round(p_away_win, 6),
        "p_polymarket_home": round(p_polymarket_home, 6),
        "p_polymarket_away": round(p_polymarket_away, 6),
    }
